...
function createCSS() {
  ...
  .menu-label { padding: 0; font-weight: 400; }
  ...
  .menu-list-item { background: ${theme.itemBackground}; color: ${theme.itemColor}; border: none; padding: 0.2rem; font-family: inherit;
    font-variant: inherit; border-radius: var(--rounded); font-weight: 400; }
  ...
  .menu-button { border: 0; background: ${theme.buttonBackground}; width: -webkit-fill-available; padding: 4px; margin: 8px; cursor: pointer; box-shadow: 4px 4px 4px 0 dimgrey;
    border-radius: var(--rounded); justify-content: center; font-family: inherit; font-variant: inherit; font-size: 1rem; font-weight: 400; }
...
class Menu {
  ...
  addInput(title, object, variable, callback) {
    const el = document.createElement('div');
    el.className = 'menu-item';
    el.innerHTML = `<input class="menu-input" type="text" id="${this.newID}" value="${object[variable]}">${title}`;
    this.container.appendChild(el);
    el.addEventListener('change', (evt) => {
